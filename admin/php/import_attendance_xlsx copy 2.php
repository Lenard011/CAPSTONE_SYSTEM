...
$response = [
    ...
    'error_messages' => [],
    'records' => [],
    'duplicate_employees' => [] // NEW: Track duplicate records with details
];
...
try {
    ...
    $duplicates = 0;
    $records = [];
    $duplicateRecords = []; // NEW: Track duplicate records
    ...
    for ($row = $startRow; $row <= $startRow + 40; $row++) {
        ...
        if ($checkStmt->rowCount() > 0) {
            $duplicates++;
            // NEW: Track duplicate record details
            $duplicateRecords[] = [
                'employee_id' => $validation['employee_id'],
                'employee_name' => $validation['employee_name'],
                'date' => $dateStr,
                'day' => $day,
                'match_method' => $validation['matched_by'] ?? 'exact_id_match'
            ];
            continue;
        }
    ...
    $response['duplicate_employees'] = $duplicateRecords; // NEW: Add duplicate records to response

    // Build message with match method
    $matchMethodDisplay = '';
    if (isset($validation['matched_by'])) {
        if ($validation['matched_by'] == 'exact_id_match') {
            $matchMethodDisplay = 'exact ID match';
        } elseif ($validation['matched_by'] == 'fuzzy_name_match') {
            $matchMethodDisplay = 'fuzzy name match';
        } else {
            $matchMethodDisplay = str_replace('_', ' ', $validation['matched_by']);
        }
    }
    ...
    if (!empty($matchMethodDisplay)) {
        $response['message'] .= " (Matched by: $matchMethodDisplay)";
    }

    // Add match method counts
    $matchMethods = array_count_values(array_column($records, 'match_method'));
    $methodMessages = [];
    if (isset($matchMethods['exact_id_match'])) {
        $methodMessages[] = "{$matchMethods['exact_id_match']} exact ID matches";
    }
    if (isset($matchMethods['fuzzy_name_match'])) {
        $methodMessages[] = "{$matchMethods['fuzzy_name_match']} fuzzy name matches";
    }
    if (!empty($methodMessages)) {
        $response['message'] .= " (" . implode(', ', $methodMessages) . ")";
    }
} catch (Exception $e) {
