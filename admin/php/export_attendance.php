...
ini_set('display_errors', 0); // Turn off display errors for Excel output
session_start();
...
if (empty($employee_id_array)) {
    ...
    header('Location: attendance.php?status=export_no_records&message=' . urlencode("No employees selected for export."));
    exit();
}

error_log("Selected " . count($employee_id_array) . " employees");
...
// =================================================================================
// --- Fetch Records for Selected Employees ---
// =================================================================================
...
$sql .= " ORDER BY employee_name ASC, date DESC";
...
try {
    ...
    header('Location: attendance.php?status=export_no_records&message=' . urlencode("Database error: " . $e->getMessage()));
    exit();
}

error_log("Total records fetched: " . count($records));
...
foreach ($records as $record) {
    ...
    $employees[$emp_id]['records'][] = $record;
}

error_log("Grouped into " . count($employees) . " employees");
...
// Remove default sheet - we'll create sheets for each employee
...
$filename .= '_' . count($employees) . '_employees.xlsx';
