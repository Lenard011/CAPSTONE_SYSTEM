...
if (empty($employee_ids)) {
    ...
    header("Location: joborderpayrolltable1.php?period=$period&cutoff=$cutoff");
    exit();
}
    ...
    try {
        $sql = "
            SELECT 
                ...
                eligibility
            FROM job_order 
            WHERE id IN ($placeholders) AND (is_archived = 0 OR is_archived IS NULL)
        ";
        ...
        foreach ($employees as &$employee) {
            ...
            // Get payroll data - handle full month vs half month
            if ($cutoff == 'full') {
                ...
                if ($payroll_data && $payroll_data['record_count'] > 0) {
                    ...
                    if ($employee['total_deductions'] == 0 && ($employee['withholding_tax'] > 0 || $employee['sss'] > 0 || $employee['philhealth'] > 0 || $employee['pagibig'] > 0)) {
                        $employee['total_deductions'] = $employee['withholding_tax'] + $employee['sss'] + $employee['philhealth'] + $employee['pagibig'];
                    }
                } else {
                ...
                $payroll_stmt = $pdo->prepare("
                    SELECT other_comp, withholding_tax, sss, philhealth, pagibig, total_deductions, gross_amount, net_amount, days_present, status
                    FROM payroll_history 
                    WHERE employee_id = ? AND employee_type = 'joborder' 
...
<body>
            ...
            <div class="payslip">
                ...
                <table class="salary-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>Monthly Salary Equivalent (22 days)</td>
                        ...
                        <?php endif; ?>
                        <?php if ($employee['philhealth'] > 0): ?>
                            <tr>
                                <td>PhilHealth Contribution</td>
                                <td class="amount" style="color: #dc2626;"><?php echo formatCurrency($employee['philhealth']); ?></td>
                            </tr>
                        <?php endif; ?>
                        <?php if ($employee['pagibig'] > 0): ?>
                            <tr>
                                <td>Pag-IBIG Contribution</td>
                                <td class="amount" style="color: #dc2626;"><?php echo formatCurrency($employee['pagibig']); ?></td>
                            </tr>
                        <?php endif; ?>
                        <?php if ($employee['withholding_tax'] == 0 && $employee['sss'] == 0 && $employee['philhealth'] == 0 && $employee['pagibig'] == 0): ?>
                            <tr>
    ...
    <div class="print-controls">
        <button class="print-btn back" onclick="window.location.href='joborderpayrolltable1.php?period=<?php echo $period; ?>&cutoff=<?php echo $cutoff; ?>'">
