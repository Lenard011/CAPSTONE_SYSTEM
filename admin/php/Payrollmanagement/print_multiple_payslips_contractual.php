...
if (empty($employee_ids)) {
    ...
    echo "<script>
        ...
        window.close();
        window.location.href = document.referrer || 'contractualpayrolltable1.php?period=$period&cutoff=$cutoff';
    </script>";
    exit();
}
...
// Debug log
error_log("Employee IDs received: " . print_r($employee_ids_array, true));
...
if (!empty($employee_ids_array)) {
    ...
    try {
        ...
        // Build SELECT query - Try both id and employee_id fields
        $conditions = [];
        $params = [];

        foreach ($employee_ids_array as $id) {
            $id = trim($id);
            if (is_numeric($id)) {
                $conditions[] = "id = ?";
                $params[] = $id;
        ...
        if ($salary_col) {
            if ($salary_col == 'rate_per_day' || $salary_col == 'daily_rate') {
                $select_fields .= ", ($salary_col * 22) as monthly_salary, $salary_col as rate_per_day";
            } else {
                $select_fields .= ", $salary_col as monthly_salary, ($salary_col / 22) as rate_per_day";
            }
        } else {
            $select_fields .= ", 0 as monthly_salary, 0 as rate_per_day";
        }

        $sql = "SELECT $select_fields FROM contractofservice WHERE ($where_clause) AND status = 'active'";
        ...
        foreach ($employees as &$employee) {
            ...
            // Get payroll data from payroll_history_contractual table
            if ($cutoff == 'full') {
                ...
                $payroll_stmt = $pdo->prepare("
                    SELECT other_comp, withholding_tax, sss, 
            ...
            if (!isset($employee['rate_per_day']) || $employee['rate_per_day'] == 0) {
                $employee['rate_per_day'] = $employee['monthly_salary'] / 22;
            }

            // Calculate total deductions if needed
            if ($employee['total_deductions'] == 0 && ($employee['withholding_tax'] > 0 || $employee['sss'] > 0)) {
                $employee['total_deductions'] = $employee['withholding_tax'] + $employee['sss'];
            }
...
error_log("Final employees_data count: " . count($employees_data));
foreach ($employees_data as $emp) {
    error_log("Employee: " . $emp['employee_id'] . " - Name: " . $emp['full_name'] . " - Gross: " . ($emp['gross_amount'] ?? 0) . " - Net: " . ($emp['net_amount'] ?? 0));
}
