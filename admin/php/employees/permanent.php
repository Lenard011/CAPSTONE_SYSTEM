...
function handleAddEmployee($conn)
{
    ...
    try {
        ...
        $employee_id = !empty($_POST['employee_id']) ? $_POST['employee_id'] : generateEmployeeId($conn);
        ...
        if ($check_result->num_rows > 0) {
            throw new Exception("Employee ID already exists. Please generate a new one.");
        }
        ...
        if ($conn->query($sql) === TRUE) {
            $conn->commit();
            $form_submit_success = "Permanent employee added successfully!";
...
if (!empty($filter_office) && $filter_office !== 'all') {
    $where_clause .= " AND office = '" . mysqli_real_escape_string($conn, $filter_office) . "'";
}
...
<body>
        ...
        <div class="main-container">
            <div class="bg-white rounded-xl shadow-lg p-4 md:p-6">
                ...
                <div class="bg-white shadow-md sm:rounded-lg overflow-hidden">
                    ...
                    <!-- Pagination -->
                    <nav class="flex flex-col md:flex-row items-center justify-between space-y-4 md:space-y-0 p-4"
                        aria-label="Table navigation">
                        <span class="text-sm font-normal text-gray-500 text-center md:text-left">
                            Showing
                        ...
                        <?php if ($total_pages > 1): ?>
                            <ul class="inline-flex -space-x-px rtl:space-x-reverse text-sm h-8">
                                <!-- Previous Button -->
                                <li>
                                    <a href="permanent.php?page=<?php echo ($current_page > 1) ? $current_page - 1 : 1; ?><?php echo ($show_inactive ? '&show_inactive=1' : '') . (!empty($filter_office) ? '&office=' . urlencode($filter_office) : ''); ?>"
                                        class="flex items-center justify-center px-3 h-8 ms-0 leading-tight text-gray-500 bg-white border border-gray-300 rounded-s-lg hover:bg-gray-100 hover:text-gray-700 <?php echo ($current_page == 1) ? 'opacity-50 cursor-not-allowed' : ''; ?>">
                                        Previous
                                    </a>
                                </li>
                                ...
                                for ($i = $start_page; $i <= $end_page; $i++):
                                    ...
                                    <li>
                                        <a href="permanent.php?page=<?php echo $i; ?><?php echo ($show_inactive ? '&show_inactive=1' : '') . (!empty($filter_office) ? '&office=' . urlencode($filter_office) : ''); ?>"
                                            class="flex items-center justify-center px-3 h-8 leading-tight text-gray-500 bg-white border border-gray-300 hover:bg-gray-100 hover:text-gray-700 <?php echo ($i == $current_page) ? 'text-blue-600 border-blue-300 bg-blue-50' : ''; ?>">
                                            <?php echo $i; ?>
                                        </a>
                                    </li>
                                <?php endfor; ?>
                                ...
                                <!-- Next Button -->
                                <li>
                                    <a href="permanent.php?page=<?php echo ($current_page < $total_pages) ? $current_page + 1 : $total_pages; ?><?php echo ($show_inactive ? '&show_inactive=1' : '') . (!empty($filter_office) ? '&office=' . urlencode($filter_office) : ''); ?>"
                                        class="flex items-center justify-center px-3 h-8 leading-tight text-gray-500 bg-white border border-gray-300 rounded-e-lg hover:bg-gray-100 hover:text-gray-700 <?php echo ($current_page == $total_pages) ? 'opacity-50 cursor-not-allowed' : ''; ?>">
                                        Next
                                    </a>
                                </li>
                            </ul>
                        ...
                        <div class="grid gap-4 mb-4">
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <button type="button" id="generateEmployeeId"
                                        class="absolute right-2.5 top-1/2 transform -translate-y-1/2 text-blue-600 hover:text-blue-800 text-sm font-medium">
                                        Generate ID
                                    </button>
                                </div>
                                <p class="text-xs text-gray-500 mt-1">Format: P-YYYY-MM-XXX (e.g., P-2024-01-001)</p>
    ...
    <script>
        ...
        function generateEmployeeId() {
            ...
            const baseId = `P-${year}-${month}`;

            // Generate a random 3-digit number for demo purposes
            // In production, you would fetch the next sequence from database
            const randomSeq = Math.floor(Math.random() * 900) + 100;
            const employeeId = `${baseId}-${String(randomSeq).padStart(3, '0')}`;
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            if (generateBtn) {
                generateBtn.addEventListener('click', generateEmployeeId);
            }
            ...
            if (payrollToggle && payrollDropdown) {
                payrollToggle.addEventListener('click', function (e) {
                    e.preventDefault();
                    const isVisible = payrollDropdown.style.display === 'block';
                    payrollDropdown.style.display = isVisible ? 'none' : 'block';
